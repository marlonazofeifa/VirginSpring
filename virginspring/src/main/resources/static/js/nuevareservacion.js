...
jQuery(document).ready(
    function($) {
        //Activamos los tooltips para los elementos con la clausula data-toggle

        $('[data-toggle="tooltip"]').tooltip();

        //Agrega funcionalidad al botón de agregar pasajero
        $(".btn_add_passenger").click(function () {
            ...
            jQuery('.field-passenger-name').keypress(function(tecla) {
                if(tecla.charCode == ",".charCodeAt(0) || tecla.charCode == ":".charCodeAt(0)) {
                    if (0 == jQuery('#invalid-character-alert').length) jQuery('#lbl-passenger-info').append("<label id = \"invalid-character-alert\" style = \"font-size: x-small; color: red;\">Ha digitado un carácter inválido (:) y (,).</label>");
                    return false;
                }
                else {
                    jQuery('#invalid-character-alert').remove();
                }
            });
        ...
        //Restringe el flujo de inserciones
        $("#arrival-date").change(function () {
            var arrivalDate = $("#arrival-date").val();

            if (arrivalDate == ""){
                $("#flight-selector, #airline-selector").attr("disabled", "disabled");
            } else {
                $("#airline-selector").attr("disabled",false);
                $("#airline-selector").focus();
            }
        });

        //Agrega funcionalidad a los selectores de aerolíneas y de fecha de llegada
        $("#airline-selector").on({
            change: function () {
                var arrivalDate = $('#arrival-date').val();
                var airline = $('#airline-selector').val();

                //Limpia, restringe y coloca una opción por defecto mientras carga los respectivos vuelos
                $('#flight-selector').empty();
                $("#flight-selector").attr("disabled","disabled");
                $('#flight-selector').append('<option>Cargando vuelos...</option>');

                $.ajax({
                    url: '/admin/nuevareservacion/obtenerVuelos',
                    type: 'GET',
                    data: {
                    ...
                    error: function () {
                        $('#flight-selector').empty();
                        $("#flight-selector").attr("disabled","disabled");
                        $('#flight-selector').append('<option>Error al cargar vuelos</option>');
        ...
        //Agrega funcionalidad al selector de vuelos
        ...
        function showConfirmationWindow(){
            ...
            var username = "";
            $.ajax({
                url: "/admin/obtener-username",
                type: "GET",
                success: function (data) {
                    username = data;

                    var selectedIndex = Array.from($("#id-partner"))[0].selectedIndex;
                    var partnerId = Array.from($('#id-partner').children())[selectedIndex].value;
                    var firstName = $("#nameRepresent").val();
                    var lastName = $("#lastname").val();
                    var arrivalDate = $("#arrival-date").val();
                    var arrivalDateArray = $("#arrival-date").val().split("-");
                    var arrivalDateString = arrivalDateArray[2] + " de";
                    switch (arrivalDateArray[1]){
                        case "01":
                            arrivalDateString += " enero ";
                            break;
                        case "02":
                            arrivalDateString += " febrero ";
                            break;
                        case "03":
                            arrivalDateString += " marzo ";
                            break;
                        case "04":
                            arrivalDateString += " abril ";
                            break;
                        case "05":
                            arrivalDateString += " mayo ";
                            break;
                        case "06":
                            arrivalDateString += " junio ";
                            break;
                        case "07":
                            arrivalDateString += " julio ";
                            break;
                        case "08":
                            arrivalDateString += " agosto ";
                            break;
                        case "09":
                            arrivalDateString += " setiembre ";
                            break;
                        case "10":
                            arrivalDateString += " octubre ";
                            break;
                        case "11":
                            arrivalDateString += " noviembre ";
                            break;
                        case "12":
                            arrivalDateString += " diciembre ";
                            break;

                    }
                    arrivalDateString += arrivalDateArray[0];
                    ...
                    for (let i = 0; i < passengersNames.length; i++){
                        passengersNames[i].value = passengersNames[i].value.replace(",","").replace(":","");
                        if (i%2 == 0){
                    ...
                    if (partnerId == "" ||
                        ...
                        if (partnerId == "")
                        {
                            $("#id-partner").css("box-shadow", "inset 0 1px 1px rgb(255, 0, 0)");
                            $("#id-partner").css("webkit-box-shadow", "inset 0 1px 1px rgb(255, 0, 0)");
                            invalidFields += "partnerId,"
                        }
                        if (firstName == "" ||
                            firstName == " " ||
                            firstName == "  ")
                            ...
                            arrivalDate == "  ")
                        {
                            ...
                            $("#arrival-date").css("box-shadow", "inset 0 1px 1px rgb(255, 0, 0)");
                            $("#arrival-date").css("webkit-box-shadow", "inset 0 1px 1px rgb(255, 0, 0)");
                        ...
                        $("#confirmation-curtain").prepend(
                            ...
                            "<div id='confirmation-header'><img src='../images/logo-canatur-color.png' width='auto' height='40em' style='float: left; '/><div><label style='line-height: 40px; margin-left: 1em;color:#03a952;font-weight: 600;'>Confirmación de reservación</label></div><div style='flex:auto; display:flex; flex-direction:column;float: right;'><h style='text-align: right;'>Fecha y hora actual:</h><label style='text-align: right;' class='reservation-document-field'>" + getTransactionDateTime() + "</label></div></div>" +
                            "<div id='confirmation-container'>" +
                            "   <div class='confirmation-container-column'></div>" +
                            "       <div class='confirmation-container-row'  style='text-align: right;font-weight:100;'><h style='flex:auto;'>Realizada por:</h><label class='reservation-document-field' style='flex: none'>"+ username.split(",")[1] + "</label></div> " +
                            "       <div class='confirmation-container-row'  style='font-weight:100;'><h>Afiliado:</h><label class='reservation-document-field' style='flex: none'>"+ Array.from($('#id-partner').children())[selectedIndex].text +"</label></div> " +
                            "       <div class='confirmation-container-row'><h>Pasajero representante:</h><label class = 'reservation-document-field'>"+ lastName + " " + firstName+"</label><h>Total de pasajeros: </h><label class = 'reservation-document-field'>" + totalPax + "</label></div> " +
                            "       <div class='confirmation-container-row'><h>Aerolínea:</h><label class = 'reservation-document-field'>"+ Array.from($("#airline-selector").children())[airlineSelected].text +"</label><h>Vuelo:</h><label class = 'reservation-document-field'>"+ flightNumber + "</label><h>Fecha de llegada:</h><label class = 'reservation-document-field' style='font-size:large; font-weight: 200;'>" + arrivalDateString + "</label><h>Hora:</h><label class = 'reservation-document-field' style='font-size:xx-large;font-weight: 200; text-align: center'>" + arrivalTime + "</label></div> " +
                        ...
                        $("#btn-salvar-reservacion").click(function () {
                            ...
                            var reservation = "{";
                            reservation += "\"id-partner\":" + "\"" + partnerId + "\"\,\n";
                            ...
                            reservation += "\"extra-passengers\":" + "\"" + passengersNamesStringReal + "\"\,\n";
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if (data == "Succesfull"){
                                        ...
                                        $("#confirmation-window").css("width","-moz-fit-content");
                                        $("#confirmation-window").css("height","fit-content");
                                        $("#confirmation-window").css("height","-moz-fit-content");
                                        $("#confirmation-window").css("min-width","fit-content");
                                        $("#confirmation-window").css("min-width","-moz-fit-content");
                                        $("#confirmation-window").css("min-height","fit-content");
                                        $("#confirmation-window").css("min-height","-moz-fit-content");
                                        $("#confirmation-window").css("padding","2em");
                                        $("#confirmation-window").append("<div style='position: relative;display: flex; flex-direction: column; align-items: center; text-align: center;'>" +
                                        ...
                                        //Vaciamos el contenedor de pasajeros
                                        $(".btn_add_passenger").empty();
                                        var svg = "<svg version=\"1.1\" id=\"Capa_1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" x=\"0px\" y=\"0px\" height=\"1.75em\" \n" +
                                            "\t viewBox=\"0 0 50 50\" style=\"enable-background:new 0 0 50 50;\" xml:space=\"preserve\">\n" +
                                            "<circle style=\"fill:#43B05C;\" cx=\"25\" cy=\"25\" r=\"25\"/>\n" +
                                        ...
                                        $("#airline-selector").attr("disabled","disabled");
                                        $("#flight-selector").attr("disabled","disabled");
        ...
        function updateTags(){
            ...
            $.ajax({
                ...
                type: 'GET',
                data: {
                    partId:$('#id-partner').find("option:selected").attr('value')
                },
                success:function (data) {
        ...
        //Obtiene la fecha de la transacción desde el lado del cliente.
        ...
        //Resetea el valor de fecha de arrivo
        function resetArrivalDate(){
            var tomorrow = new Date(new Date().getTime() + 24 * 60 * 60 * 1000);
            var defaultDate = tomorrow.getFullYear() + "-";
            if (Math.floor(tomorrow.getMonth()+1/10)==0)
                defaultDate += "0";
            defaultDate += tomorrow.getMonth() + 1 + "-";
            if (Math.floor(tomorrow.getDate()/10) == 0){
                defaultDate += "0";
            }
            defaultDate += tomorrow.getDate();
            $("#arrival-date").val(defaultDate);
            $("#arrival-date").change();
