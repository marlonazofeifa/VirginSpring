...
(this,function(exports,module){
    ...
    var SEPERATOR=',';
    var COPY_PROPS='placeholder pattern spellcheck autocomplete autocapitalize autofocus accessKey accept lang minLength required'.split(' ');
    function tagsInput(input){
        ...
        function getValue(){
            return $('.tag',true).map(function(tag){return tag.textContent;}).concat(base.input.value || []).join(SEPERATOR);
        }

        function setValue(value){
            $('.tag',true).forEach(
                function(t){return base.removeChild(t);});
        ...
        var base = document.getElementById('tags-input');
        if( base == null || base.length == 0) {
            base = createElement('div','tags-input');
        }
        var sib= input.nextSibling;
        ...
        base.input.setAttribute('maxLength','20');
        ...
        input.addEventListener('focus',function(){base.input.focus();});
        ...
        base.input.addEventListener('keydown',function(e){
            ...
            if(key === ENTER || key === COMMA || key === TAB){
                ...
                key === DELETE && selectedTag){if(selectedTag.nextElementSibling !== base.input)select(selectedTag.nextElementSibling);
                base.removeChild(selectedTag);
                setInputWidth();
                save();
            }else if(key === BACKSPACE){
                    if(selectedTag){
                        select(selectedTag.previousElementSibling);
                        base.removeChild(selectedTag);
                        setInputWidth();
                        save();
                    }else if(last && pos === 0){
                    ...
                    if(selectedTag){
                        if(selectedTag.previousElementSibling){
                            select(selectedTag.previousElementSibling);
                        }
                    }else if(pos !== 0){
                        return;
                ...
                if(!selectedTag)return;
                    select(selectedTag.nextElementSibling);
                }else {
                    return select();
                }
                e.preventDefault();
